...
from computer_helper import check_lower_scores
  ...
  def take_turn(self):
    ...
    for k,v in self.lower_section.items():
      if self.lower_section[k] == None:
        ...
        pass
    found_box = check_lower_scores(dice_counts, bot_available)
    if found_box != "None":
      want_reroll = False
      if found_box == "Yahtzee":
        score_yahtzee(self, dice_counts)
      elif found_box == "LG Straight":
        score_lgstraight(self, dice_counts, False)
      elif found_box == "SM Straight":
        score_smstraight(self, dice_counts, False)
      elif found_box == "Full House":
        score_full_house(self, dice_counts, False)
